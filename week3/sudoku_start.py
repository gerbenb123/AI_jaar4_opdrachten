...
def solve(grid):
    ...
    for x in grid:
        #check of waarde nog niet is ingevuld.
        if len(grid[x]) > 1: 
            for n in grid[x]:
...
def arc_consistency(grid, c, v):
    #je zou maar duidelijke comments neerzetten zodat de leerling niet 
    # de hele avond aan het kloten is om de peers te fixen.....

    # for x in grid:
    #     if len(grid[x]) == 1: 
    #         #hier dus de waarde die al ingevuld is. weghalen van de andere waarden.
    #         row = x[0]
    #         column = x[1]
    #         value = grid[x]
    #         #print(row, value)
    ...
    #             if len(grid[m]) > 1:
    #                 #remove value from all variables in same row.
    #                 if m[0] == row:
    #                     grid[m]=grid[m].replace(value,'')
    #                 #remove value from all variables in same column.
    #                 if m[1] == column:
    #                     grid[m]=grid[m].replace(value,'')
    #                 #remove value from all variables in same block.
                    
    #                 if (row == 'A' or row == 'D' or row== 'G'):
    #                     if column in range(3):
    #                         if m[0] == row and (int(m[1]) in range(3)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+1) and (int(m[1]) in range(3)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+2) and (int(m[1]) in range(3)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                     if column in range(4,6):
    #                         if m[0] == row and (int(m[1]) in range(4,6)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+1) and (int(m[1]) in range(4,6)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+2) and (int(m[1]) in range(4,6)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                     if column in range(7,9):
    #                         if m[0] == row and (int(m[1]) in range(7,9)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+1) and (int(m[1]) in range(7,9)):
                                
    #                             grid[m]=grid[m].replace(value,'')
    #                         if m[0] == chr(ord(row)+2) and (int(m[1]) in range(7,9)):
    ...
    if new_grid != grid:
        for key, value in grid.items():
            if len(str(value)) == 1 and key != c:
                if not arc_consistency(grid, key, value):
                    return False
